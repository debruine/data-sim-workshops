...
You can change the mean and SD. Simulate a lot of values (1e5 == 100,000) and visualise them with `hist()`.
...
```{r}
dat3_pois <- dat3 |>
  mutate(A_pois = norm2pois(A, 4, 1, 0.5),
         B_pois = norm2pois(B, 4, 2, 1),
         C_pois = norm2pois(C, 4, 3, 1.5))

get_params(dat3_pois)
```
...
```{r}
guess2 <- rnorm_multi(50, vars = 12, r = 0.5, empirical = TRUE)
...
```{r}
# full correlation matrix as a vector
#         X1   X2   X3   X4
