...
sub_version_sd <- 20 # SD for the subjects' random slope for version
...
version_eff      <- 50  # mean difference between versions: incongruent - congruent
cond_version_ixn <-  0  # interaction between version and condition


dat <- add_random(sub_id = sub_n) %>%
  add_random(stim_id = stim_n) %>%
  add_between(.by = "sub_id", 
              sub_cond = c("easy","hard")) %>%
  add_contrast(col = "sub_cond",
               colnames = "sub_cond.e") %>%
  add_within(.by = "stim_id", 
             stim_version = c("congruent", "incongruent")) %>%
  add_contrast(col = "stim_version",
               colnames = "stim_version.e") %>%
  add_ranef(.by = "sub_id",
            sub_i = sub_sd,
            sub_version_slope = sub_version_sd,
            .cors = sub_i_version_cor) %>%
  add_ranef(.by = "stim_id",
            stim_i = stim_sd,
            stim_version_slope = stim_version_sd,
            stim_cond_slope = stim_cond_sd,
            stim_cond_version_slope = stim_cond_version_sd,
            .cors = stim_cors) %>%
  add_ranef(error = error_sd) %>%
  mutate(rt = grand_i + sub_i + stim_i + # intercepts
          (cond_eff + stim_cond_slope) * sub_cond.e + # condition effect
          (version_eff + stim_version_slope + sub_version_slope) * stim_version.e + # version effect
          (cond_version_ixn + stim_cond_version_slope) * sub_cond.e * stim_version.e + # condition * version interaction
           error
         ) %>%
  select(sub_id, stim_id, sub_cond, sub_cond.e, stim_version, stim_version.e, rt)
...
```{r lmer}
mod <- lmer(rt ~ sub_cond * stim_version +
              (1 | sub_id) + 
              (1 | stim_id),
            data = dat)
