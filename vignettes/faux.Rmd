...
output: 
  rmarkdown::html_document:
    df_print: paged
vignette: >
  %\VignetteIndexEntry{Intro to Faux}
...
utm <- ggMarginal(ut, type = "histogram")

ul <- ggplot(dat_norta, aes(U, L)) +
  geom_point(alpha = 0.1) +
  geom_smooth(method = lm, formula = y ~ x) +
  labs(x = "Uniform",
       y = "Likert") +
  scale_x_continuous(breaks = 0:10)
ulm <- ggMarginal(ul, type = "histogram")

tl <- ggplot(dat_norta, aes(T, L)) +
  geom_point(alpha = 0.1) +
  geom_smooth(method = lm, formula = y ~ x) +
...
```{r, include=FALSE}
dat_r <- cor(attitude)
dat_mu <- colMeans(attitude)
dat_sd <- var(attitude) |> diag() |> sqrt() # colSDs
